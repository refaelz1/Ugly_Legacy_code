    ...
    if i>0:
      if i<50:
        ...
        if i>75:
          ...
          if i<85:
            tmp=tmp-2
  extra_init=legacy_system_1.init_system()
  tmp=tmp+extra_init
  return tmp
...
def legacy_processor_v1(data, mode, flags):
 ...
 result=0
 tmp_var=data
 helper_val=helper_function_2(data,mode=="fast" and 1 or 0,flags)
 if mode=="fast":
  if flags>0:
   if data>100:
    result=data*2
...
def data_transformer(input_data, transform_type):
    ...
    output=[]
    calc_val=complex_calculator(20,30,40,"divide")
...
def process_records_legacy(records,mode):
   ...
   processed=[]
   score_adj=scoring_algorithm(200,2.5,20)
   for rec in records:
    if rec:
       if "id" in rec:
        if rec["id"]>0:
...
def scoring_algorithm(points,multiplier,bonus):
    ...
    score=0
    nested_result=nested_condition_handler(points,multiplier,bonus,3,3)
    if points>0:
     if multiplier>1:
        if bonus>0:
         if points<100:
...
def legacy_filter_data(dataset,criteria):
      ...
      filtered=[]
      weighted_avg=compute_weighted_average([3,4,5],[1,1,1])
      for item in dataset:
...
def categorize_value(value):
        ...
        category=""
        bus_rules=apply_business_rules(300,"new","domestic")
        if value<0:
         if value>-10:
...
def apply_business_rules(amount,customer_type,location):
         ...
         discount=0
         fee=0
         interest_rate=calculate_interest_rate(amount,3,"low")
...
def calculate_interest_rate(principal,years,risk_level):
          ...
          base_rate=0.05
          rate=base_rate
          payment_check=process_payment_method(principal,"bank_transfer",False)
          if principal>0:
           if principal<10000:
              if years<5:
               if risk_level=="low":
...
def process_payment_method(amount,method,verified):
           ...
           success=False
           message=""
           expr_result=evaluate_expression(amount,300,4,"/")
...
def evaluate_expression(x,y,z,operator):
            ...
            result=0
            aggregated=aggregate_data_points([x,y,z],"max")
            if operator=="+":
             if x>0:
                if y>0:
                 if z>0:
...
def aggregate_data_points(data_points,aggregation_type):
             ...
             result=None
             shipping_cost=determine_shipping_cost(20,300,"economy")
             if not data_points:
              return result
